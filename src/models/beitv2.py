...
def preprocess_beitv2_state_dict(state_dict:Dict[str, torch.Tensor], n_layers:int=None) -> Dict[str, torch.Tensor]:
    """Adjusts the keys of the state_dict of the BEiTv2 model by removing obsolete weights.

    Args:
        state_dict (Dict[str, torch.Tensor]): The state_dict with the pretrained weights of the BEiTv2 model.
        n_layers (int, optional): The number of layers to reuse. If all layers (12) should be used, set to None. Defaults to None.

    Returns:
        Dict[str, torch.Tensor]: The adjusted state_dict.
    """    
    new_sd = {}
    for key in list(state_dict.keys()):
        ...
        if 'blocks' in key:
            ...
            if n_layers is not None and block_idx > n_layers-1:
                continue
        new_sd[key] = state_dict[key]
    return new_sd

def get_beitv2_model(state_dict_path:os.PathLike, n_layers:int=12) -> VisionTransformerForMaskedImageModeling:
    """Creates a pretrained BEiTv2 model.

    Args:
        state_dict_path (os.PathLike): Path to the state dict of the BEiTv2 model.
        n_layers (int, optional): The number of layers to reuse. Has a maxiumum of 12 layers. Defaults to 12.

    Returns:
        VisionTransformerForMaskedImageModeling: The pretrained BEiTv2 model.
    """
    beitv2_kwargs = {
        "drop_path_rate": 0.1,
    ...
    beit2 = VisionTransformerForMaskedImageModeling(
        patch_size=16, embed_dim=768, depth=n_layers, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **beitv2_kwargs
    )

    state_dict = torch.load(state_dict_path)['model']
    state_dict = preprocess_beitv2_state_dict(state_dict, n_layers=n_layers)
    result = beit2.load_state_dict(state_dict)
